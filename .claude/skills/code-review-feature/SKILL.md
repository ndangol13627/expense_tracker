...
description: Performs a parallel security and quality code review for a specific Spendly feature by analyzing the current Git diff against the feature specification. Runs dedicated security and maintainability reviewers concurrently, consolidates their findings into a prioritized action plan, and asks for approval before making any code changes. Use this whenever the user asks to review, audit, inspect, or validate a Spendly feature before committing, or invokes commands like "/code-review-feature <spec-name>".
allowed-tools:
  - Bash(git diff)
